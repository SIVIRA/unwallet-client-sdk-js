...
export class DAuth
{
  ...
  private ASSETS_CONTRACT_ACCOUNT_NAME = 'pidassets111';
  ...
  private initEventListener(): void
  {
    window.addEventListener('message', async (ev: MessageEvent) => {
      ...
      try {
        const result = await this.routes.get(action.method)!.call(this, action.args);
  ...
  public async createAssetTransferTransaction(receiverID: string, assetSourceID: number, quantity: string, memo: string = ''): Promise<SignedTransaction>
  {
    ...
    const sig = await this.signTransaction(this.ASSETS_CONTRACT_ACCOUNT_NAME, 'transfer', data);
  ...
  public signTransaction(contract: string, action: string, data: Uint8Array): Promise<string>
  {
    return new Promise((resolve, reject) => {
      const chan = new MessageChannel();
      chan.port1.onmessage = (ev: MessageEvent) => {
